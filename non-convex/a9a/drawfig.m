...
load('GTTAC1.mat');%加载保存的迭代梯度信息x_k_store{}
load('GTSPIDER1.mat');%加载保存的迭代解信息gradient()
load('CENSPIDER1.mat');%加载保存的邻接矩阵信息C_store
load('GTTAC2.mat');%加载保存的迭代梯度信息x_k_store{}
load('GTSPIDER2.mat');%加载保存的迭代解信息gradient()
load('CENSPIDER2.mat');%加载保存的邻接矩阵信息C_store
load('GTTAC3.mat');%加载保存的迭代梯度信息x_k_store{}
load('GTSPIDER3.mat');%加载保存的迭代解信息gradient()
load('CENSPIDER3.mat');%加载保存的邻接矩阵信息C_store
...
% figure(3);
% plot(1:Maxgen1,const1,'r-x','linewidth',1),hold on;
% plot(1:Maxgen1,const2,'k-^','linewidth',1),hold on;
% plot(1:Maxgen1,const3,'b-o','linewidth',1),hold on;
% legend('DstoFW','DenFW','CenFW');
