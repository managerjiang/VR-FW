...
listx_i=[]
#X_i=np.zeros(x_dim)
X_i=np.load('F:/anaconda_spyder/non-convex/data/classify/xi0_a9a.npy')
#X_i=np.ones(x_dim)
...
for t in range(T):
    ...
    for j in range(agent_n):                
        if C[rank][j] != 0:
            comm.send(X_i, dest=j, tag=rank+20)
    neigh_x1=[0]*agent_n    
    for j in range(agent_n):
        if C[rank][j] != 0:
            neigh_x1[j]=comm.recv(source=j, tag=j+20)
    sumx1=[0]*x_dim
    for j in range(agent_n):
        sumx1=sumx1+C[rank][j]*neigh_x1[j]
    '''    
    if t%q==0: #########
        v1=gradient_com(X_i)
    else:
        ...
        #batch_size=4*q*q#20*(q-(t%q)) #20*(q-t//q) ##可调节参数，采样个数
...
if rank==0:  
    ...
    runntime=end-start
    print(runntime)
    #np.save('F:/anaconda_spyder/non-convex/data/classify/runntime_GTSPIDER.npy',runntime)

    X_col=np.array(X_col)
    #np.save('F:/anaconda_spyder/non-convex/data/classify/X_col_GTSPIDER.npy',X_col) # 保存为.npy格式

    file_name='GTSPIDER3.mat'
    savemat(file_name,{'runtime_spider3':runntime,'x_col_spider3':X_col})
'''    
    consenseq=[]
    fvseq=[]
    for t in range(T):
